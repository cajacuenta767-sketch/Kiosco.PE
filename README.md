...
> "Sencillo" es lo que la bodeguera pide cuando le pagan con billete grande, y es lo que esta app promete ser.
> El bodeguero no sabe cuánto gana. No sabe qué se le está acabando hasta que un cliente se lo pide. Y el cuaderno de fiados
> se pierde, se moja o se olvida. Sencillo reemplaza el cuaderno con algo tan simple como WhatsApp.

## Qué hace hoy (v0.8)

| Módulo | Qué resuelve |
|---|---|
| **Bienvenida** | Primer arranque: nombre de la bodega y elegir entre catálogo de ejemplo, bodega de ejemplo con dos semanas de movimiento, empezar de cero o vincular un celular que ya usa Sencillo. |
| **Vender** | Cobrar en 3 toques. Cada producto con su dibujo o foto, ordenados por lo más vendido, búsqueda escrita o **por voz**, escáner de código de barras con la cámara, venta rápida sin producto, calculadora de vuelto (también sin registrar venta), Efectivo / Yape / Plin / Tarjeta / Fiado. Precios por paquete (six-pack, docena). "Lo de siempre" de cada cliente en un toque. Al cobrar con Yape o Plin, el QR y el número de la bodega en grande para que el cliente escanee. Deshacer la última venta y sonido al cobrar. |
| **Stock** | Qué tienes, qué se acaba, qué está agotado. Ingreso de mercadería, ganancia por producto, historial de movimientos de cada producto, **fechas de vencimiento por lote** con aviso "véndelo primero" y baja por merma, **lista de precios** con dibujos para WhatsApp o imprimir, y **pedido sugerido** al proveedor según la rotación de 14 días. |
| **Fiados** | El cuaderno de fiados que no se pierde. Deuda por cliente, abonos, historial, fecha de pago acordada con aviso de vencimiento, **tope de fiado** por cliente con aviso al cobrar, y recordatorio por WhatsApp con un toque. |
| **Caja** | Cuánto vendiste y cuánto ganaste hoy (neto de gastos), por método de pago. Gastos del día. Últimos 7 días. Resumen del mes. Comprobante de venta y **resumen de la semana** por WhatsApp. Cierre de caja con cuadre de efectivo y **aviso de noche** si quedó abierta. |
| **Cuenta y acceso** | Registro y login desde la bienvenida: cuenta con el celular de la dueña y un PIN, sin correo. Entra desde cualquier teléfono, aunque haya perdido el suyo. Segundo celular por código de 6 dígitos o con número y PIN. Ve sus celulares conectados y cierra la sesión del perdido; el celular cerrado lo avisa y ofrece entrar de nuevo. Bloqueo con PIN al abrir la app, con entrada de ayudante sin PIN y recuperación con el PIN de la cuenta. |
...
docker build -t sencillo . && docker run -p 3000:3000 sencillo   # PWA + API en un contenedor
```
